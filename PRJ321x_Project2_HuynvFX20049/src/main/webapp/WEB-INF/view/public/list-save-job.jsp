    ...
    <div class="container">
        <div class="row">
            <div class="col-lg-12 pr-lg-5">
                <div class="row">
                	<c:forEach var="tempSaveJob" items="${saveJobs}">
                        <div class="col-md-12 ">
                            <div class="job-post-item p-4 d-block d-lg-flex align-items-center">
                                <div class="one-third mb-4 mb-md-0">
                                    <div class="job-post-item-header align-items-center">
                                        <span class="subadge">${tempSaveJob.recruitment.type}</span>
                                        <h2 class="mr-3 text-black" ><a href="${pageContext.request.contextPath}/post/detailPost?id=${tempSaveJob.recruitment.id}">${tempSaveJob.recruitment.title}</a></h2>
                                    </div>
                                    <div class="job-post-item-body d-block d-md-flex">
                                        <div class="mr-3"><span class="icon-layers"></span> <a href="${pageContext.request.contextPath}/company/detailCompany?id=${tempSaveJob.recruitment.company.id}">${tempSaveJob.recruitment.company.companyName}</a></div>
                                        <div><span class="icon-my_location"></span> <span>${tempSaveJob.recruitment.address}</span></div>
                                    </div>
                                </div>
                                <input type="hidden" id="idRe${tempSaveJob.recruitment.id}" value="${tempSaveJob.recruitment.id}">
                                <div class="one-forth ml-auto d-flex align-items-center mt-4 md-md-0">
                                    <div>
<%--                                         <a href="${'/save-job/delete/'}+${tempSaveJob.id}" class="icon text-center d-flex justify-content-center align-items-center icon mr-2"> --%>
<!--                                             <span class="icon-delete"></span> -->
<!--                                         </a> -->
                                        <a onclick="save(${tempSaveJob.recruitment.id})" class="icon text-center d-flex justify-content-center align-items-center icon mr-2">
                                            <span class="icon-delete"></span>
                                        </a>
                                    </div>
                                    <a  data-toggle="modal" data-target="#exampleModal${tempSaveJob.recruitment.id}" class="btn btn-primary py-2">Apply Job</a>
                                </div>
                            </div>
                        </div><!-- end -->
                        <!-- Modal -->
                        <div class="modal fade" id="exampleModal${tempSaveJob.recruitment.id}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                            <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title" id="exampleModalLabel">Ứng tuyển: <span>${tempSaveJob.recruitment.title}</span></h5>
                                    ...
                                    <form method="post" action="/user/apply-job">
                                        <div class="modal-body">
                                            <div class="row">
                                                <div class="col-12">
                                                    <select id="choose${tempSaveJob.recruitment.id}" onchange="choosed(${tempSaveJob.recruitment.id})" class="form-control" aria-label="Default select example">
                                                        <option selected>Chọn phương thức nộp</option>
                                                        <option value="1">Dùng cv đã cập nhật</option>
                                                        <option value="2">Nộp cv mới</option>
                                                    </select>
                                                </div>
                                                <div id="loai1${tempSaveJob.recruitment.id}" style="display:none" class="col-12">
                                                    <label for="fileUpload" class="col-form-label">Giới thiệu:</label>
                                                    <textarea rows="10" cols="3" class="form-control" id="text${tempSaveJob.recruitment.id}" ></textarea>
                                                </div>
                                                <div id="loai2${tempSaveJob.recruitment.id}" style="display:none" class="col-12">
                                                    <label for="fileUpload" class="col-form-label">Chọn cv:</label>
                                                    <input type="file" class="form-control" id="fileUpload${tempSaveJob.recruitment.id}" name="file"   required>
                                                    <label for="fileUpload" class="col-form-label">Giới thiệu:</label>
                                                    <textarea rows="10" cols="3" class="form-control" id="text${tempSaveJob.recruitment.id}" ></textarea>
                                                </div>
                                            </div>
                                            <div class="modal-footer">
                                                ...
                                                <button type="button" id="button1${tempSaveJob.recruitment.id}" style="display: none" onclick="apply1(${tempSaveJob.recruitment.id})" class="btn btn-primary">Nộp</button>
                                                <button type="button" id="button2${tempSaveJob.recruitment.id}" style="display: none" onclick="apply(${tempSaveJob.recruitment.id})" class="btn btn-primary">Nộp</button>
                                            </div>
                                        </div>
                                    </form>
<!--                                    <form method="post" action="/user/apply-job">-->
...
<!--                                    </form>-->
                ...
                <c:if test="${saveJobs.size() == 0}">
                	<div style="text-align: center">
                    	<p style="color:red;">Bạn chưa lưu công việc nào!</p>
                	</div>
                </c:if>
                <div class="row mt-5">
        ...
        $.ajax(
            {
                ...
                success: function (data) {
                    ...
                    if (isOk) {
                    	setInterval(() => {
                        	window.location.href = "${pageContext.request.contextPath}/user/listSaveJob";
                        	console.log('Reload');
                        	//location.reload();
        				}, 3000);
                    }
