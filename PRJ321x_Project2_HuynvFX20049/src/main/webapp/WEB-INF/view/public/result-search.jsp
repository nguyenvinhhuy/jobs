    ...
    <div class="container">
        ...
        <div class="row">
            <div class="col-lg-12 pr-lg-5">
                <div class="row">
                	<c:forEach var="tempRecruitmentByKey" items="${recruitmentByKey}">
                        <div class="col-md-12 ">
                            <div class="job-post-item p-4 d-block d-lg-flex align-items-center">
                                <div class="one-third mb-4 mb-md-0">
                                    <div class="job-post-item-header align-items-center">
                                        <span class="subadge">${tempRecruitmentByKey.type}</span>
                                        <h2 class="mr-3 text-black" ><a href="${pageContext.request.contextPath}/post/detailPost?id=${tempRecruitmentByKey.id}">${tempRecruitmentByKey.title}</a></h2>
                                    </div>
                                    <div class="job-post-item-body d-block d-md-flex">
                                        <div class="mr-3"><span class="icon-layers"></span> <a href="${pageContext.request.contextPath}/company/detailCompany?id=${tempRecruitmentByKey.company.id}">${tempRecruitmentByKey.company.companyName}</a></div>
                                        <div><span class="icon-my_location"></span> <span>${tempRecruitmentByKey.address}</span></div>
                                    </div>
                                </div>
                                <input type="hidden" id="idRe${tempRecruitmentByKey.id}" value="${tempRecruitmentByKey.id}">
                                <c:if test="${user.role.id == 2}">											
                                <div class="one-forth ml-auto d-flex align-items-center mt-4 md-md-0">
                                    <div>
                                        <a onclick="save(${tempRecruitmentByKey.id})" class="icon text-center d-flex justify-content-center align-items-center icon mr-2">
                                            <span class="icon-heart"></span>
                                        </a>
                                    </div>
                                    <a  data-toggle="modal" data-target="#exampleModal${tempRecruitmentByKey.id}" class="btn btn-primary py-2">Apply Job</a>
                                </div>
                                </c:if>
                            </div>
                        </div><!-- end -->
                        <!-- Modal -->
                        <div class="modal fade" id="exampleModal${tempRecruitmentByKey.id}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                            <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title" id="exampleModalLabel">Ứng tuyển: <span>${tempRecruitmentByKey.title}</span></h5>
                                    ...
                                    <form method="post" action="/user/apply-job">
                                        <div class="modal-body">
                                            <div class="row">
                                                <div class="col-12">
                                                    <select id="choose${tempRecruitmentByKey.id}" onchange="choosed(${tempRecruitmentByKey.id})" class="form-control" aria-label="Default select example">
                                                        <option selected>Chọn phương thức nộp</option>
                                                        <option value="1">Dùng cv đã cập nhật</option>
                                                        <option value="2">Nộp cv mới</option>
                                                    </select>
                                                </div>
                                                <div id="loai1${tempRecruitmentByKey.id}" style="display:none" class="col-12">
                                                    <label for="fileUpload" class="col-form-label">Giới thiệu:</label>
                                                    <textarea rows="10" cols="3" class="form-control" id="text${tempRecruitmentByKey.id}" >
                                                    </textarea>
                                                </div>
                                                <div id="loai2${tempRecruitmentByKey.id}" style="display:none" class="col-12">
                                                    <label for="fileUpload" class="col-form-label">Chọn cv:</label>
                                                    <input type="file" class="form-control" id="fileUpload${tempRecruitmentByKey.id}" name="file" required>
                                            ...
                                            <div class="modal-footer">
                                                ...
                                                <button type="button" id="button1${tempRecruitmentByKey.id}" style="display: none" onclick="apply1(${tempRecruitmentByKey.id})" class="btn btn-primary">Nộp</button>
                                                <button type="button" id="button2${tempRecruitmentByKey.id}" style="display: none" onclick="apply(${tempRecruitmentByKey.id})" class="btn btn-primary">Nộp</button>
                                            </div>
                                        </div>
                                    </form>
                                </div>
                    ...
                    <c:if test="${tempRecruitmentByKey.size() == 0}">
